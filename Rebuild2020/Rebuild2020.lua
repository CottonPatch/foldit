...
function DefineGlobalVariables()
  ...
  --         Reset_g_bSegmentsAlreadyRebuiltTable()
	...
	--         bSegmentIsAllowedToBeRebuilt() and Step1_Rebuild1Puzzle()
  ...
  -- in the Step1_Rebuild1Puzzle protein) in DefineGlobalVariables() then the user can
  ...
  -- locked and ligand segments in Step1_Rebuild1Puzzle(). This is important because we
  ...
  -- For informational display in log file only.
	...
	-- Used in SetClashImportance() and Step7_WiggleSelected()
  ...
  -- Used in 5 functions
	...
	-- Used in bSegmentIsAllowedToBeRebuilt() and Step3_Rebuild1SegmentRangeSet()
  
	g_QuickSaveStackPosition = 60 -- Uses slot 60 and higher...
  -- Used in 3 functions
  
	g_RebuildClashImportance = 0
	-- Used in Step4_Rebuild1SegmentRange()
  ...
  -- Used in Step1_Rebuild1Puzzle() and Step3_Rebuild1SegmentRangeSet(),
  ...
  g_Stats_Script_NumberOfAttempts_MutateSidechainsAll = 0
	...
	-- Used in AskUserToSelectMoreOptions() and Step1_Rebuild1Puzzle()
  ...
  -- Used in 3 functions
  -- see Step1_Rebuild1Puzzle() for definition
  ...
  if g_bDebugMode == true then
	...
	-- Used in Step8_MutateSideChainsOfSelectedSegments() and AskUserToSelectMutateOptions()
...
-- Temporarily disable normal condition checking...
  ...
  -- Compute the maximum potential bonus points (not available in beginner puzzles)
    ...
    tonumber(l_ScoreWithPotentialBonusPoints) - tonumber(l_ScoreWithoutPotentialBonusPoints)
  ...
  -- script.
  g_Score_ScriptBest = GetPoseTotalScore() -- g_Score_ScriptBest is a string!
  ...
  save.Quicksave(3) -- Save -- I doubt this is used, but whatever
...
function SetupLocalDebugFuntions()
 ...
 --   WiggleAll and Step7_WiggleSelected...
  ...
  current.RandomlyChange_g_Debug_CurrentEnergyScore = function()
    ...
    -- ShakeSidechainsSelected,WiggleAll,Step7_WiggleSelected
    ...
    -- Update Recent Best
    if g_Debug_CurrentEnergyScore > g_Debug_RecentBest then
      g_Debug_RecentBest = g_Debug_CurrentEnergyScore
    end
  ...
  recentbest.GetEnergyScore = function()
    ...
    --if g_Debug_RecentBest ~= nil then -- should never be nil!
      --local l_GetPoseTotalScore = GetPoseTotalScore()
      --if tonumber(l_GetPoseTotalScore) > tonumber(g_Debug_RecentBest) then
        --g_Debug_CurrentEnergyScore = l_GetPoseTotalScore
      --else
        g_Debug_CurrentEnergyScore = g_Debug_RecentBest
      --end      
      --No!! g_Debug_RecentBest = nil
    --else
      -- g_Debug_ScriptBestEnergyScore is always the script
      -- best, but not necessarily the saved recentbest.
      --g_Debug_CurrentEnergyScore = g_Debug_ScriptBestEnergyScore
    --end
	end
	recentbest.Save = function()
    -- If you say so...
    g_Debug_RecentBest = tonumber(GetPoseTotalScore())
    --no!! g_Debug_ScriptBestEnergyScore = g_Debug_RecentBest   
    
    -- Don't we already save the recent best as g_Debug_ScriptBestEnergyScore?
    -- Well, g_Debug_ScriptBestEnergyScore is always the *script* best, but g_Debug_RecentBest
    -- can be less than the best pose. While working on improved ScorePart poses, we go back to
    -- less than best poses to try to improve them. Saving a current pose to recentbest, allows
    -- us to work on the less than best pose while its score goes up and down, whithout having
    -- to go all the way back to the best pose if one foldit action lowers our score.
    -- It should be noted that, if we save the current pose as recentbest and we improve the
    -- recent best, then the recentbest will be updated with the improvements. See
    -- RandomlyChange_g_Debug_CurrentEnergyScore() where we update recentbest if it improves.
  end
  


  g_Debug_bAllowMaxRandomness = true -- default is false, which allows pregressively 
  --                                    less randomness the longer the script runs.

	structure = {}
	g_Debug_NumberOfTimesWeCalled_MutateSidechainsAll = 0
  structure.MutateSidechainsAll = function(l_Iterations)
    
    if g_Debug_bAllowMaxRandomness == true then
  ...
  -- Called from 13 places...
    ...
    g_Debug_QuickSaveEnergyScore[l_IntegerSlot] = GetPoseTotalScore()
    return
  end 
...
function DisplayPuzzleProperties() -- was SetPuzzleProperties()
  ...
  local l_Current_PoseTotalScore = GetPoseTotalScore()
    ...
    if tonumber(l_Current_PoseTotalScore) > 0 then
      g_DensityWeight = 
        (tonumber(l_Current_PoseTotalScore)
       - g_ComputedMaximumPotentialBonusPoints
       - tonumber(l_Score_TotalOfAllSegmentsIncludingLigands)
       - 8000)
       / tonumber(l_DensityTotal)
    end
...
function Add_Loop_Helix_And_Sheet_Segments_To_SegmentRangesTable() -- was FindAreas()
  -- Called from Step2_Rebuild1Run() when l_How = 'segments'
...
function bIsADisulfideBondSegment(l_SegmentIndex) -- was IsBridge()
  -- Called from DisulfideBonds_GetCount()...
	...
	local DisulfidesEnergySubscore =
		tonumber(PaddedNumber(current.GetSegmentEnergySubscore(l_SegmentIndex, l_ScorePart), 1, 3))
...
function bSegmentIsAllowedToBeRebuilt(l_SegmentIndex)
  ...
  -- g_bUserSelectd_SegmentsAllowedToBeRebuiltTable[] is populated in Step1_Rebuild1Puzzle() from 
  ...
  -- locked and ligand segments in Step1_Rebuild1Puzzle()...
  ...
  if g_CurrentRebuildPointsGained >
    ...
    -- g_CurrentRebuildPointsGained is added to in Step3_Rebuild1SegmentRangeSet()
...
function Calculate_ScorePart_Score(l_ScorePart_Name, l_StartSegment, l_EndSegment) -- was getPartscore
  ...
  local l_DirtyScoreTotal = 0 -- anytime a float is involved in a calulation, even simple addition, 
  --                             it can become dirty! e.g.; 1.123 becomes 1.1230000000001
	local l_CleanScoreTotal = 0
  local l_SegmentEnergyScore
    ...
    --       1-4, 2-5, 3-6 in the second run, and so on...
      ...
      --       this is only for a small range of segments, not all segments...
			l_DirtyScoreTotal = l_DirtyScoreTotal + g_SegmentScoresTable[l_SegmentIndex]
		end
    l_CleanScoreTotal = PaddedNumber(l_DirtyScoreTotal, 1, 3)
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  end

	if l_ScorePart_Name == 'total' then
    
    -- Example usage: from Step9_CheckForScorePartImprovements,
    --                when stepping through each ScorePart to update the ScorePart_Scores field...
		l_CleanScoreTotal = GetPoseTotalScore()
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  ...
  if l_ScorePart_Name == 'loctotal' then --total segment scores
    ...
    --        Calculate_SegmentRange_Score(nil, l_StartSegment, l_EndSegment)
		...
		l_CleanScoreTotal = Calculate_SegmentRange_Score(nil, l_StartSegment, l_EndSegment)
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  ...
  if l_ScorePart_Name == 'ligand' then --ligand score
      ...
      l_SegmentEnergyScore = current.GetSegmentEnergyScore(l_SegmentIndex)
			l_DirtyScoreTotal = l_DirtyScoreTotal + l_SegmentEnergyScore        
		end
    l_CleanScoreTotal = PaddedNumber(l_DirtyScoreTotal, 1, 3)
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  ...
  l_CleanScoreTotal = Calculate_SegmentRange_Score(l_ScorePart_Name, l_StartSegment, l_EndSegment)
	return l_CleanScoreTotal -- yes we are returning a string on purpose!
...
function Calculate_SegmentRange_Score(l_ScorePart_NameOrTable, l_StartSegment, l_EndSegment) -- GetSubScore
  ...
  --                             it can become dirty! e.g.; 1.123 becomes 1.1230000000001
	local l_CleanScoreTotal = 0
  local l_SegmentRange_Score, l_SegmentEnergyScore, l_SegmentEnergySubScore
  ...
  -- A table of ScoreParts was passed in...
    ...
    -- only include the ScoreParts of the passed in list of ScoreParts...
		for l_ScorePart_NameOrTableIndex = 1, #l_ScorePart_NameOrTable do
			...
			l_SegmentRange_Score = Calculate_SegmentRange_Score(l_ScorePart_Name, l_StartSegment, 
                                                       l_EndSegment)
			l_DirtyScoreTotal = l_DirtyScoreTotal + l_SegmentRange_Score
		end
    l_CleanScoreTotal = PaddedNumber(l_DirtyScoreTotal, 1, 3)
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  end
  ...
  -- Nothing was passed in...
    ...
    local l_CleanScoreTotal = GetPoseTotalScore(l_pose)
    return l_CleanScoreTotal -- yes we are returning a string on purpose!
  end 
  ...
  --    when stepping through each ScorePart to update the ScorePart_Scores field...
  for l_SegmentIndex = l_StartSegment, l_EndSegment do
    l_SegmentEnergySubScore = current.GetSegmentEnergySubscore(l_SegmentIndex, l_ScorePart_Name)
    l_DirtyScoreTotal = l_DirtyScoreTotal + l_SegmentEnergySubScore
  end
  l_CleanScoreTotal = PaddedNumber(l_DirtyScoreTotal, 1, 3)
  return l_CleanScoreTotal -- yes we are returning a string on purpose!
...
function CheckForLowStartingScore()
  ...
  local l_Current_PoseTotalScore = GetPoseTotalScore()
    ...
    if g_bHasDensity == false then
		...
		local l_WeightedDensitySubScore = tonumber(l_DensitySubScore) * (g_DensityWeight + 1)
		local l_ScoreWithoutElectronDensity = tonumber(l_Current_PoseTotalScore) - l_WeightedDensitySubScore
	...
	if tonumber(l_Current_PoseTotalScore) >= l_LowScore then
...
function ConvertAllSegmentsToLoops()
  -- Called from Step3_Rebuild1SegmentRangeSet()...
  ...
  -- of at the beginning and end of every Segment Range Set.
    ...
    --                                Step3_Rebuild1SegmentRangeSet() with a 
...
function DisplayDetailsOfIncludedSegments()
  ...
  local l_SkippedSegmentRanges -- maybe?
 	...
 	g_OriginallySelectedSegmentRanges = FindSelectedSegmentRanges() -- only used in CleanUp()
  local l_ListOfSelectedSegmentRanges =
    ConvertSegmentRangesTableToListOfSegmentRanges(g_OriginallySelectedSegmentRanges)
  if l_ListOfSelectedSegmentRanges == "" then
    l_ListOfSelectedSegmentRanges = "none"
  end
...
function GetPoseTotalScore(l_pose) -- was Score()
  ...
  -- A pose includes the main protein's structure and any ligands (adjacent molecules).
	...
	local l_DirtyScoreTotal = l_pose.GetEnergyScore()
  l_CleanScoreTotal = PaddedNumber(l_DirtyScoreTotal, 1, 3)
  --l_CleanScoreTotal = tonumber(l_PaddedNumber) -- a simple call to tonumber() can turn a pretty number
  --                                                like 123.456 to 123.4559999999999 or 123.456000000001
  return l_CleanScoreTotal -- yes, we are returning a string on purpose. Stupid dirty floats.

end -- function GetPoseTotalScore(l_pose) -- was Score()
...
function NormalConditionChecking_TemporarilyReEnableToCheckScore() -- was FilterOn()
  ...
  local l_Current_PoseTotalScore = GetPoseTotalScore()
	if tonumber(l_RecentBest_PoseTotalScore) > tonumber(l_Current_PoseTotalScore) then
...
function Populate_g_SegmentScoresTable_BasedOnUserSelectedScoreParts() -- was GetSegmentScores()
  -- Called from Populate_g_XLowestScoringSegmentRangesTable() -- was FindWorst()
	...
	local l_SegmentScore, l_Reference_ScorePart, l_Density_ScorePart, l_bThisSegmentIsMutable
    ...
    -- make sure this is not a frozen, locked or ligand segment...
        ...
        --                  weighted Density_ScorePart
				...
				l_bThisSegmentIsMutable = structure.IsMutable(l_SegmentIndex)
      ...
      -- Step9_CheckForScorePartImprovements()
      --l_SegmentScore = tonumber(PaddedNumber(l_SegmentScore, 1, 3))
      l_SegmentScore = PaddedNumber(l_SegmentScore, 1, 3) -- yes, we store it as a string!
...
function Populate_g_XLowestScoringSegmentRangesTable(l_RecursionLevel) -- was FindWorst()
  -- Called from Step2_Rebuild1Run() and 
  --             recursively below...
...
function QuickSaveStack_LoadLastSavedSolution() -- was PopPosition()
  -- Called from 3 functions
...
function QuickSaveStack_RemoveLastSavedSolution() -- was ClrTopPosition()
...
function QuickSaveStack_SaveCurrentSolution() -- was PushPosition()
  -- Called from 3 functions
...
function Reset_g_ScorePart_Scores_Table() -- was ClearScores()
  -- Called from Step4_Rebuild1SegmentRange()...
...
function SaveBest() -- <-- Updates g_Score_ScriptBest
  ...
  if g_bUserSelected_NormalConditionChecking_DisableForThisEntireScriptRun == true then
    ...
    -- 2) Reenabling normal condition checking would not likely improve our g_Score_ScriptBest.
  	...
  	local l_PotentialScore_IfAllConditionsAreMet =
          tonumber(l_PoseTotalScore_WithConditionChecking_Disabled) +
          g_UserSelected_MaximumPotentialBonusPoints
    
    if (l_PotentialScore_IfAllConditionsAreMet <= tonumber(g_Score_ScriptBest)) then
      return
    end
  ...
  local l_Current_PoseTotalScore = GetPoseTotalScore()
  local l_Real_PointsGained = tonumber(l_Current_PoseTotalScore) - tonumber(g_Score_ScriptBest)
  ...
  if l_Real_PointsGained >= l_MinimumGain_ForSave or 
    ...
    -- Important !!!
    -- Important !!!
    g_Score_ScriptBest = l_Current_PoseTotalScore  -- yes, g_Score_ScriptBest is a string. Get over it.
    -- Important !!!
    -- Important !!!
    
    if g_bUserSelected_FuseBestScorePartPose == false and tonumber(g_Score_ScriptBest) > 0 then
...
function FindUniqueScorePartPoses() -- was ListSlots()
  -- Called from Step3_Rebuild1SegmentRangeSet()
  
  -- For each of the segment range rebuild attempts/rounds that changed the protein's
  -- structure, we checked if any of the protein's ScorePart scores improved. If any of
  -- the ScorePart scores improved we saved the protein's pose to the foldit quicksave
  -- slot with the same number as the ScorePart number. And we saved the improved ScorePart 
  -- score and PoseTotalScore in the g_ScorePartScoresTable.
  
  -- Note: The ScorePart_Number is being used as both a ScorePart identifier and a foldit
  -- Quicksave slot number of where the improved ScorePart pose is stored.
  
  -- Before we shake, wiggle and mutate each of the *unique* poses stored in the ScorePart
  -- quicksave slots for improvements, we need to identify which of the poses are unique.
  -- It is very likely that more than one quicksave slot will have the same pose. When one 
  -- ScorePart improves, usually one or more other ScoreParts also improve. And when this 
  -- happens, the same pose is stored in each of the corresponding ScorePart quicksave slots.
  
  -- This function, FindUniqueScorePartPoses(), identifies which ScorePart quicksave slots
  -- have the same pose. Identifying the unique poses will prevent us from attempting to 
  -- improve the exact same pose more than once.      
   
  -- In this function we will update the following two fields of the g_ScorePartScoresTable:
  --   1) FirstInString, and 
  --   2) StringOfScorePartNumbersWithSamePoseTotalScore
  
  -- The StringOfScorePartNumbersWithSamePoseTotalScore will look something like this example:
  -- 1=5=7 2=3=9 4 6=8
  -- In this example, 
  --  ScoreParts 1, 5 and 7 have the same PoseTotalScore, 
  --  ScoreParts 2, 3 and 9 have the same PoseTotalScore and 
  --  ScoreParts 6 and 8 have the same PoseTotalScore
  -- Allow me to explain how these different ScoreParts can end up having the same PoseTotalScore:
  -- After each rebuild attempt of one segment range (normally 10 attempts/rounds per segment
  -- range) one or more ScoreParts might improve. For each of the score parts that improve
  -- for a particular rebuild we associate the same pose total score of that rebuild to each
  -- of the improved score parts.
  -- For example:
  -- 1) We complete rebuild round 1 of 10, and scoreparts 2, 3 and 9 are the only 3 of the
  --    20ish scoreparts to improve. In this case we update the corresponding ScorePart rows of 
  --    the g_ScorePartScoresTable with the current PoseTotalScore.
  -- 2) Next, we complete round 2 of 10 and only score parts 1, 5 and 7 improve. So we
  --    assign the latest PoseTotalScore to score parts 1, 5 and 7.
  -- 3) Next, we complete round 3 of 10 and only score parts 6 and 8 improve. Again, we
  --    assign the latest PoseTotalScore, this time, to score parts 6 and 8.
  -- 4) Skip a few rounds and finally, we complete round 4 of 10 and only score part 4 improved.
  --    So we assign the latest PoseTotalScore to score part 4.
  -- 5) Now, at this point if we were to build a string of score part numbers with the same
  ...
  -- Basically, we assume if the PoseTotalScore is the same, then those poses must also be the same.
  -- Odd assumption? Maybe, but it's probably correct more than 99.9% of the time.
  -- 
	-- For each g_ScorePartScoresTable row with a unique PoseTotalScore (or the first of a group of
  -- rows with the same PoseTotalScore) we set the bFirstInStringOfScorePartNumbersWithSamePoseTotalScore
  ...
  --                           bFirstInStringOfScorePartNumbersWithSamePoseTotalScore=5}
	for l_TableIndex = 1, #g_ScorePartScoresTable do
      ...
      -- This field will be checked in Step3_Rebuild1SegmentRangeSet()...
      ...
      -- If all of the ScoreParts have the same pose, we can abbreviate the list of ScoreParts as "All"
      if l_NumberOfScorePartsIncluded >= #g_ScorePartScoresTable then
        l_StringOfScorePartNumbersWithSamePoseTotalScore = "All" -- this will save us space in the log file
      end
      ...
      l_Combined_StringOfScorePartNumbersWithSamePoseTotalScore ..
		...
		end -- if l_ScorePartScoresDoneStatusTable[l_TableIndex] == false then
    
	end -- for l_TableIndex = 1, #g_ScorePartScoresTable do

	print(l_Combined_StringOfScorePartNumbersWithSamePoseTotalScore)
  local l_DebugMe = 1

end -- FindUniqueScorePartPoses()
-- ...end of Support Functions.
-- Start of My Favorite Functions...
function Step1_Rebuild1Puzzle() -- was DRW()
  -- Called from 1 place in xpcall()...
  -- Calls Step2_Rebuild1Run() was DRcall()
  ...
  DisplayDetailsOfIncludedSegments()
    ...
    -- Not! what you are looking for! See below...
		Step2_Rebuild1Run("drw") -- was DRcall()
  ...
  for l_RunCycle = 1, g_UserSelected_NumberOfRunCycles do
    ...
    -- Important!!!
    -- Important!!!
		Step2_Rebuild1Run("drw") -- below; was DRcall()
    -- Important!!!
    -- Important!!!
    -- Important!!!
		
		-- Uncomment the methods you want to use...
		-- Step2_Rebuild1Run("all")
		-- Step2_Rebuild1Run("areas")
		-- Step2_Rebuild1Run("fj")
		-- Step2_Rebuild1Run("simple")
...
end -- Step1_Rebuild1Puzzle() -- was DRW()
function Step2_Rebuild1Run(l_How) -- was DRcall()
  -- Called from Step1_Rebuild1Puzzle() above; was DRW()
  -- Calls Step3_Rebuild1SegmentRangeSet() below; was DeepRebuild()
 ...
 -- In the grand scheme of things, see below --->>>We Are Here Now<<<---
  ...
  -- ...End of Script  
	...
	if l_How == "drw" then
      ...
      -- Important!!!
      -- Important!!!
			Step3_Rebuild1SegmentRangeSet() -- below; was DeepRebuild()
      -- Important!!!
      -- Important!!!
      -- Important!!!
...
end -- Step2_Rebuild1Run(l_How) -- was DRcall()
function Step3_Rebuild1SegmentRangeSet() -- was DeepRebuild()
  -- Called from Step2_Rebuild1Run() above; was DRcall()
  -- Calls Step4_Rebuild1SegmentRange() below; was ReBuild()
  --       Step10_WorkMoreOnImprovedScorePartPoses below
  ...
  if g_RunCycle == 0 then    
	...
	if g_bUserSelected_ConvertAllSegmentsToLoops == true then
		ConvertAllSegmentsToLoops() -- why do we do this for every range set? why not just once at top of script
    -- Remember loops are not helices. Loops are just plain swiggly lines...
	end
  save.Quicksave(3) -- I doubt this used, but whatever
  recentbest.Save() -- Probably not needed, but whatever
  ...
  --                           bFirstInStringOfScorePartNumbersWithSamePoseTotalScore=5}
    ...
    -- Important!!!
    -- Important!!!
    l_NumberOfTimesStructureChanged = Step4_Rebuild1SegmentRange(l_StartSegment, l_EndSegment)--was ReBuild(
    -- Important!!!
    -- Important!!!
    -- Important!!!
    ...
    if l_NumberOfTimesStructureChanged > 0 then
      ...
      -- Important!!!
      -- Important!!!
      Step10_WorkMoreOnImprovedScorePartPoses(l_StartSegment, l_EndSegment)
      -- Important!!!
      -- Important!!!
      -- Important!!!
      ...
      if g_bUserSelected_FuseBestScorePartPose == true then
        ...
        if g_bUserSelected_MutateBeforeFuseBestScorePartPose == true and
           g_bUserSelected_MutateAfterStabilizePosesOfSelectedScoreParts == false then
            ...
            -- Important!!!
            -- Important!!!
            Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, "BeforeFuse") -- below
        ...
        -- Important!!!
        -- Important!!!
        Step14_FuseBestScorePartPose(l_StartSegment, l_EndSegment) -- below; was Fuze()
        -- Important!!!
        -- Important!!!
        -- Important!!!
        ...
        if g_bUserSelected_MutateAfterFuseBestScorePartPose == true then
          ...
          -- Important!!!
          -- Important!!!
          Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, "AfterFuse") -- below
          -- Important!!!
          -- Important!!!
          -- Important!!!
        end
    ...
    --SaveBest() -- SaveBest() would have already been called from Shake, Wiggle or Mutate above.
    save.Quickload(3) -- Why not call recentbest.Restore() here instead of save.Quickload(3)? Whatev.
    ...
    -- Important!!!
    -- Important!!!
		SetSegmentsAlreadyRebuilt(l_StartSegment, l_EndSegment) -- was AddDone()
    -- Important!!!
    -- Important!!!
    -- Important!!!
    ...
    local l_Score_After_SeveralMoreChangesToSegmentRange = GetPoseTotalScore()
		...
		g_CurrentRebuildPointsGained = tonumber(l_Score_After_SeveralMoreChangesToSegmentRange) -
                                   tonumber(l_Score_Before_SeveralChangesToSegmentRange)
...
end -- Step3_Rebuild1SegmentRangeSet() -- was DeepRebuild()
function Step4_Rebuild1SegmentRange(l_StartSegment, l_EndSegment) -- was ReBuild()
  -- Called from: Step3_Rebuild1SegmentRangeSet() above; was DeepRebuild()
  
  -- Calls: Step5_RebuildSelectedSegments() below; was localRebuild()
  --        Step6_ShakeSelected() below
  --        Step7_WiggleSelected() below
  --        Step8_MutateSideChainsOfSelectedSegments() below
  --        Step9_CheckForScorePartImprovements() way below
  ...
  recentbest.Restore() -- can't hurt. just note though, this is not in the original code.
    ...
    -- Important!!!
    -- Important!!!
		l_bStructureChanged = Step5_RebuildSelectedSegments(l_StartSegment, l_EndSegment) -- localRebuild()
    -- Important!!!
    -- Important!!!
    -- Important!!!
    
    -- NEW!!! We are looking for ANY change to the structure, 
    -- with a positive OR negative score change!!!
    if l_bStructureChanged == true then
      ...
      -- Important!!!
      -- Important!!!
      Step6_ShakeSelected("AfterRebuild1") -- FromWhere; below
      -- Important!!!
      -- Important!!!
      -- Important!!!
      ...
      if g_bUserSelected_ExtraShakeAndWiggles_AfterRebuild == true then
        ...
        -- Important!!!
        -- Important!!!
        Step6_ShakeSelected("AfterRebuild2") -- FromWhere; below
        Step7_WiggleSelected(2, false, true, "AfterRebuild2") -- Iterations,w/Bb,w/SC,FromWhere; below
        -- Important!!!
        -- Important!!!
        -- Important!!!
        ...
        -- Important!!!
        -- Important!!!
        Step6_ShakeSelected("AfterRebuild4") -- FromWhere; below
        Step7_WiggleSelected(4, true, false, "AfterRebuild3") --Iterations,bWBackbone,bWSideChains,FromWhere
        -- Important!!!
        -- Important!!!
        -- Important!!!
      ...
      if g_bUserSelected_MutateAfterRebuild == true then
        ...
        -- Important!!!
        -- Important!!!
        Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, "AfterRebuild") -- below
        -- Important!!!
        -- Important!!!
        -- Important!!!
      ...
      -- We have just rebuilt (and optionally, mutated, shaked and wiggled) a single segment range
      -- for the n-th time, where n is a number from 1 to l_MaxRounds. 
      
      -- Next, let's check for ScorePart improvements by calling Step9_CheckForScorePartImprovements():
      -- In that function, for each score part that improved during this rebuild round we will do
      -- the following:
      -- 1) Update the corresponding ScorePart row of the g_ScorePartScoresTable with the improved
      --    ScorePart_Score and the Current_PoseTotalScore (which may or may not have improved)
      -- 2) Update the corresponding ScorePart FoldIt Quicksave slot with the current pose (which
      --    may or may not have improved. Usually not.)
      -- After all of the rebuild rounds have completed for this specific SegmentRange we will shake
      -- and wiggle each of the saved ScorePart-improved poses to hopefully find more points.
          
      -- Important!!!
      -- Important!!!
      -- Important!!!      
      Step9_CheckForScorePartImprovements(l_StartSegment, l_EndSegment) -- was SaveScores()
      -- Important!!!
      -- Important!!!
      -- Important!!!
      ...
      -- l_bStructureChanged == false here!
     
      -- There was no change to the structure. At this point we stop processing this segment 
      -- range and move on to the next one...
     
      -- We will rarely enter this section anymore, now that we allow large negative scores
      -- to continue on to stabilize...
  ...
  -- After all the rounds are done for this segment range, let's
  -- make sure we are still working with the best pose...
  if g_bSketchBookPuzzle == false then -- was SKETCHBOOKPUZZLE
    -- Make sure we do not miss an improvement during rebuild
    local Score_RecentBest = GetPoseTotalScore(recentbest)
    local Score_PoseTotal = GetPoseTotalScore()
    if  tonumber(Score_RecentBest) > tonumber(Score_PoseTotal) then -- was if RBScore() > bestScore
        ...
        local l_Score_After_Restore = GetPoseTotalScore()
        if tonumber(l_Score_After_Restore) > g_Score_ScriptBest + 0.00001 then
            -- I can't imagine this ever happening, because nothing ever gets 
            -- past g_Score_ScriptBest!!!
...
end -- Step4_Rebuild1SegmentRange() -- was ReBuild()
function Step5_RebuildSelectedSegments(l_StartSegment, l_EndSegment) -- was localRebuild()
  -- Called from Step4_Rebuild1SegmentRange() above; was Rebuild()
  ...
  for l_CurrentAttemptForThisRound = 1, l_MaxAttemptsPerRound do
    ...
    l_ScoreImprovement = tonumber(l_Score_After_Rebuild) - tonumber(g_Score_ScriptBest)
...
end -- Step5_RebuildSelectedSegments() -- was localRebuild()
function Step6_ShakeSelected(l_FromWhere) -- was Wiggle()
  ...
  -- iterations; therefore, we always set iterations = 1...
...
end -- Step6_ShakeSelected(l_FromWhere)
function Step7_WiggleSelected(l_Iterations, l_bWBackbone, l_bWSideChains, l_FromWhere) -- was Wiggle()
...
end -- Step7_WiggleSelected(l_ClashImportance, l_FromWhere)
function Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, l_FromWhere)--doMutate+Mutat
  -- Called from 3 functions above and below
    ...
    return Step8b_MutateSideChainsAll(l_FromWhere)
  end  
...
end -- Step8_MutateSideChainsOfSelectedSegments()
function Step8b_MutateSideChainsAll(l_FromWhere) -- was doMutate() + MutateAll()
  -- Called from Step8_MutateSideChainsOfSelectedSegments() above
...
end -- Step8b_MutateSideChainsAll(l_FromWhere)
function Step9_CheckForScorePartImprovements(l_StartSegment, l_EndSegment) -- was SaveScores()
  -- Called from 2 functions
  
  -- We have just rebuilt (and optionally, shaked, wiggled and mutated, only one segment
  ...
  -- all these rebuild attempts, in Step3_Rebuild1SegmentRangeSet() above we will step
  -- through each of these best saved score part poses and shake, wiggle and mutate them
  ...
  --   StringOfScorePartNumbersWithSamePoseTotalScore=4,
  --   bFirstInStringOfScorePartNumbersWithSamePoseTotalScore=5
  --{{ 4,-2070.481,-2070.481,"",false},{ 5,4.297,-8544.629,"",false},
  -- { 6,   41.990,-5500.114,"",false},{ 7,0.456,-4424.680,"",false},
  -- { 8,    1.309,-5500.114,"",false},{ 9,0.390,-8544.629,"",false},
  -- {10,    1.322,-4424.680,"",false},{11,0.385,-4424.680,"",false},
  -- {12,    1.375,-5500.114,"",false},{13,1.206,-4424.680,"",false},
  -- {14,    0.209,-5500.114,"",false}}
  
	-- Create a new list of active ScoreParts, then call Calculate_ScorePart_Score() to get each
  -- ScorePart's score...
  ...
  -- note: each SegmentRange has several rebuild rounds.
  
  -- For each score part that improved during this rebuild round do the following:
  -- 1) Update the corresponding ScorePart row of the g_ScorePartScoresTable with the improved
  --    ScorePart_Score and the Current_PoseTotalScore (which may or may not have improved)
  -- 2) Update the corresponding ScorePart FoldIt Quicksave slot with the current pose (which
  --    may or may not have improved. Usually not.)
  ...
  -- note: several g_ScorePartScoresTable rows can have the same l_Current_PoseTotalScore.
  ...
  --                           bFirstInStringOfScorePartNumbersWithSamePoseTotalScore=5}
	for l_TableIndex = 1, #g_ScorePartScoresTable do
		...
		if tonumber(l_NewScorePart_Score) > tonumber(l_OldScorePart_Score) then
      
        -- Update the corresponding ScorePart row of the g_ScorePartScoresTable with the improved
        -- ScorePart_Score and the Current_PoseTotalScore (which may or may not have improved)
      ...
      -- Update the corresponding ScorePart FoldIt Quicksave slot with 
      -- the current pose (which may or may not have improved. Usually not.
      ...
      -- Important!!!
      -- Important!!!
      -- See Step3_Rebuild1SegmentRangeSet() for the corresponding save.Quickload()
...
function Step10_WorkMoreOnImprovedScorePartPoses(l_StartSegment, l_EndSegment) -- was in DeepRebuild()
  ...
  --       Step7_WiggleSelected above
  --       Step8_MutateSideChainsOfSelectedSegments above
  ...
  -- We will now shake, wiggle and mutate each of the *unique* poses stored in the ScorePart
  -- quicksave slots to see if we can improve any of them. I say *unique* because more than
  -- one quicksave slot will likely have the same pose. This is because when one ScorePart
  -- improves, often one or more other ScoreParts also improve. And when this happens, the 
  -- same pose is stored in each of the corresponding ScorePart quicksave slots. We call
  -- FindUniqueScorePartPoses() to identify which ScorePart quicksave slots have the same
  -- pose. This way we don't attempt to improve the exact same pose more than once.      
  
  local l_Score_Before_SeveralChangesToSegmentRange = g_Score_ScriptBest

  print("                     " ..
        "Preparing to work more on the following improved score part poses...")
  FindUniqueScorePartPoses() -- was ListSlots()     
  ...
  for l_ScorePart_Scores_TableIndex = 1, #g_ScorePartScoresTable do -- was Scores[]
    ...
    if l_bFirstInASet == true then
      ...
      l_ScorePart_Number = g_ScorePartScoresTable[l_ScorePart_Scores_TableIndex]
      ...
      if l_StringOfScorePartNumbersWithSamePoseTotalScore == "All" then 
        ...
        g_ScorePartText = " ScorePart" .. l_Plural .. ":" ..
      ...
      -- Important!!!
      -- Important!!!
      -- See Step9_CheckForScorePartImprovements() below save.Quicksave(l_ScorePart_Number) -- "Save"
      ...
      -- See Step9_CheckForScorePartImprovements()
      ...
      if g_bUserSelected_StabilizePosesOfSelectedScoreParts == true then          
        ...
        -- Important!!!
        -- Important!!!
        Step11_Stabilize1PoseOfSelectedScoreParts(l_StartSegment, l_EndSegment) -- below; was qStab()
        -- Important!!!
        -- Important!!!
        -- Important!!!
        ...
        -- Important!!!
        -- Important!!!
        Step6_ShakeSelected("SimpleS&W")  -- FromWhere; below
        -- Important!!!
        -- Important!!!
        -- Important!!!
        ...
        -- Important!!!
        -- Important!!!
        Step7_WiggleSelected(1, false, true, "SimpleS&W") --Iterations,bWBackbone,bWSideChains,FromWhere
        -- Important!!!
        -- Important!!!
        -- Important!!!
      ...
      if g_bUserSelected_MutateAfterStabilizePosesOfSelectedScoreParts == true then
        ...
        -- Important!!!
        -- Important!!!
        Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, "AfterStabilize")
        -- Important!!!
        -- Important!!!
        -- Important!!!
      ...
      --No!!! l_Current_ImprovedScorePart_PoseTotalScore = GetPoseTotalScore()
      
      if tonumber(l_Current_ImprovedScorePart_PoseTotalScore) > 
            tonumber(l_Best_ImprovedScorePart_PoseTotalScore) then
        
        -- After we finish checking every unique ScorePart pose for improvements
        -- we will reload the one best improved ScorePart pose for Fuse...
  ...
  -- Re-load the most improved ScorePart pose...
  -- Important!!!
  -- Important!!!
  -- Important!!!
  ...
  --no!local l_Score_After_SeveralChangesToSegmentRange = GetPoseTotalScore()
  local l_Score_After_SeveralChangesToSegmentRange = l_Best_ImprovedScorePart_PoseTotalScore
   
  local l_PotentialPointLoss = tonumber(l_Score_Before_SeveralChangesToSegmentRange) -
                               tonumber(l_Score_After_SeveralChangesToSegmentRange)
  
  local l_MaxLossAllowed = g_UserSelected_RejectEachRebuildWithAPotentialLossExceedingXPoints * 
                          (l_EndSegment - l_StartSegment + 1) / 3
  if l_PotentialPointLoss > l_MaxLossAllowed then
    
    print("The most improved score part pose's potential loss of " .. 
        l_PotentialPointLoss .. " points is " ..
        "greater than Max Loss Allowed of " .. PaddedNumber(l_MaxLossAllowed, 1, 1) .. "." ..
        " Restoring resent best pose for Fuse..." )
    recentbest.Restore()
...
end -- function Step10_WorkMoreOnImprovedScorePartPoses()
function Step11_Stabilize1PoseOfSelectedScoreParts(l_StartSegment, l_EndSegment) -- was qStab()
  -- Called from Step3_Rebuild1SegmentRangeSet() above

  -- The following assumes that 80 to 90% of the time the pose at the beginning
  -- of this function is not the script's best pose. This is because we are
  -- trying to further improve the poses from the improved ScoreParts from the
  -- many rounds we attempted to rebuild one segment range. So, let's save the
  -- current pose to the stack. If, at the end of this function, our pose does
  -- not improve, we will restore this pose from the stack.
  -- We do this because if our score goes up, we want to keep the better pose,
  -- but if our score goes down we don't want to go all the way back to the 
  -- script best pose rather just back to the pose at the beginning of this
  -- function. This allows us to keep working on the ScorePart pose without
  -- constantly going back to the script best pose as the score goes up and
  -- down from calling Shake, Wiggle and Mutate. The calls to recentbest.Save()
  -- and recentbest.Restore() in this function do not negatively affect this 
  -- logic. In this function recentbest.Restore() will either keep the better
  -- pose from WiggleAll, or go back to the pose just before the call to WiggleAll.
  -- Still, it does not go back to the script best pose.
  
  local l_Score_Before_Stabilize = GetPoseTotalScore()
  
  QuickSaveStack_SaveCurrentSolution() -- now PushPosition()

	SetClashImportance(0.1)
  Step6_ShakeSelected("Stabilize")
	
	if g_bUserSelected_MutateDuringStabilizePosesOfSelectedScoreParts == true then
		SetClashImportance(1)
    Step8_MutateSideChainsOfSelectedSegments(l_StartSegment, l_EndSegment, "Stabilize")
	end
	
	if g_bUserSelected_ExtraStabilizePosesOfSelectedScoreParts == true then -- default is false
    
    SetClashImportance(0.4)
    Step12_WiggleAll(1, "ExtraStabilize") -- Iterations, FromWhere
    
		-- Note the third parameter uses the global value instead of the local value here...
    SetClashImportance(1)
    Step6_ShakeSelected("ExtraStabilize")
  ...
  recentbest.Save()
	SetClashImportance(1)
  Step12_WiggleAll(3, "Stabilize") -- Iterations, FromWhere
  local l_Score_After_Wiggle = GetPoseTotalScore()
  recentbest.Restore() -- if WiggleAll reduced our score, go back to the pose just before WiggleAll  
  
  local l_Score_After_Restore = GetPoseTotalScore()

  -- If the above actions did not improve our pose, then go back to the pose 
  -- from the beginning of this function...
  if tonumber(l_Score_After_Restore) < tonumber(l_Score_Before_Stabilize) then
    QuickSaveStack_LoadLastSavedSolution() -- now PopPosition()
  else 
    QuickSaveStack_RemoveLastSavedSolution() -- now ClrTopPosition()
  end
  l_Score_After_Stabilize2 = GetPoseTotalScore()
  local l_DebugMe = 1

end -- function Step11_Stabilize1PoseOfSelectedScoreParts(l_StartSegment, l_EndSegment)
function Step12_WiggleAll(l_Iterations, l_FromWhere) -- was Wiggle()
  -- Called from 5 functions above and below
...
end -- function Step12_WiggleAll(l_ClashImportance, l_FromWhere)
function Step14_FuseBestScorePartPose(l_StartSegment, l_EndSegment) -- was Fuze()
  -- Called from Step3_Rebuild1SegmentRangeSet() above

  -- The following assumes that 80 to 90% of the time the pose at the beginning of this function is not 
  -- the script's best pose. See also notes at the top of Step11_Stabilize1PoseOfSelectedScoreParts().
  
  -- I don't know why this function uses Quicksave slot 4 and Stabilize uses the QuickSaveStack
  -- slots 60 and above, but whatever...
  
  -- I also don't know why we use a combination of Quicksave slot 4 and recentbest.Save/Restore instead
  -- of just using one or two Quicksave slots or even better, just use the QuickSaveStack instead...
  ...
  local l_Score_During_Fuse = GetPoseTotalScore()
  save.Quicksave(4)
  recentbest.Save()

	Fuse1(0.3, 0.6) -- ClashImp_Before, ClashImp_After
  Fuse3()
  l_Score_During_Fuse = FuseUpdateSlot4(l_Score_During_Fuse)
  
	Fuse2(0.3, 1) -- ClashImportance_Before, ClashImportance_After
  recentbest.Restore() -- Will keep current if better. Will only go back to the beginning of Fuse if worse
  ...
  recentbest.Restore() -- Will keep current if better. Will only go back to the beginning of Fuse if worse
  --SaveBest() -- would already have been called from ShakeSelected() or WiggleAll()
  FuseUpdateSlot4(l_Score_During_Fuse) -- we don't care about the return value on this last call to update
  recentbest.Restore() -- Will keep current if better. Will only go back to the beginning of Fuse if worse
  --SaveBest() -- would already have been called from ShakeSelected() or WiggleAll()
...
function Fuse2(l_ClashImportance_Before, l_ClashImportance_After) -- was Fuze2()
  ...
  Step12_WiggleAll(1, "Fuse2a") -- Iterations, FromWhere
	
  SetClashImportance(1)
  Step12_WiggleAll(1, "Fuse2b") -- Iterations, FromWhere
...
function Fuse3() -- was FuzeEnd()
  ...
  SetClashImportance(1)
  Step12_WiggleAll(3, "Fuse3") -- Iterations, FromWhere
...
function FuseUpdateSlot4(l_Score_Before) -- was reFuze()
  
  local l_Score_After = GetPoseTotalScore()
  if l_Score_After > l_Score_Before then
    save.Quicksave(4)
    return l_Score_After
  end
  
  save.Quickload(4) -- it's probably already loaded, but whatever
  return l_Score_Before
  
end
-- ...end of My Favorite Functions.
-- Start of Clean Up functions...
function CleanUp(l_ErrorMessage)
  -- Called from Step1_Rebuild1Puzzle() above or xpcall() below
...
function CleanUpSelectedSegmentRanges() -- was SetSelection()
  ...
  -- g_OriginallySelectedSegmentRanges is populated in DisplayDetailsOfIncludedSegments() way above
	if g_OriginallySelectedSegmentRanges ~= nil then
    
		for l_SetIndex = 1, #g_OriginallySelectedSegmentRanges do
      
			local l_StartSegment = g_OriginallySelectedSegmentRanges[l_SetIndex][sst_StartSegment]
			local l_EndSegment = g_OriginallySelectedSegmentRanges[l_SetIndex][sst_EndSegment]
			selection.SelectRange(l_StartSegment, l_EndSegment)
...
function ScriptDocumentation()
  -- Called from 0 places. Just stuck it in a fuction so I could fold it (hide it in the editor)...
	...
	4. It will not try a Step14_FuseBestScorePartPose if the loss is too great depending on the size of the rebuild.
	...
	13. Step14_FuseBestScorePartPose and StabilizePosesOfSelectedScoreParts can be suppressed (this is the default, if the score is negative from the start)
	...
	2.8.2 Add call to CleanUp after Step1_Rebuild1Puzzle stops normally.
...
xpcall(Step1_Rebuild1Puzzle, CleanUp) -- run in protected mode, so we can fail gracefully, by calling CleanUp()...
--Step1_Rebuild1Puzzle() -- Call Step1_Rebuild1Puzzle() directly when debugging to more
